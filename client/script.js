...
const helloColorChange = (event) => {
    sayHelloButton.style.backgroundColor ='black'
    sayHelloButton.style.color = 'white'
}

sayHelloButton.addEventListener('mouseover', helloColorChange)
...
const helloColorChangeBack = (event) => {
    sayHelloButton.style.backgroundColor = '#EFEFEF'
    sayHelloButton.style.color = 'black'
}

sayHelloButton.addEventListener('mouseout', helloColorChangeBack)
    ...
    axios.get('http://localhost:3000/animals')
        .then(res => {
            for(let i = 0; i < res.data.length; i++){
                let ohMyElement = document.createElement('p')
...
const animalButton = document.querySelector('#animals-button')

animalButton.addEventListener('click', ohMy)
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/hello-all')
        .then(res => {
            console.log(res.data)
            let repeatContent = document.querySelector('#repeat-text')
            repeatContent.textContent = res.data
        })
    }


const rptButton = document.querySelector('#repeat-button')

rptButton.addEventListener('click', repeatMyParam)
...
const queryFunction = () => {
    axios.get('http://localhost:3000/query-test?spell=idk-if-this-is-right')
        .then(({data}) => console.log(data))
}

const qrButton = document.querySelector('#query-button')

qrButton.addEventListener('click', queryFunction)
